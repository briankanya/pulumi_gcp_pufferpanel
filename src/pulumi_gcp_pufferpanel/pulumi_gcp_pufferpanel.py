...
from pulumi_gcp.config import project, zone  # type: ignore[attr-defined]
...
class PufferPanel(ComponentResource):
    ...
    def __init__(
        ...
        Args:
            ...
            opts (Optional[ResourceOptions]): Optional pulumi resource options for this created resource.
            server_name (str): The name to use for the preemptible compute instance.
        """
        super().__init__(t="PufferPanel", name=name, opts=opts)
        self._code_bucket = self._create_code_bucket()
        self._code_bucket_object = self._create_code_bucket_object(bucket=self._code_bucket)
        self._dns_name = dns_name
        self._dns_zone = dns_zone
        self._disk_size = disk_size
        self._disk_type = disk_type
        self._machine_image = self._get_image(
            image_family=self.IMAGE_FAMILY, image_project=self.IMAGE_PROJECT
        )
        self._machine_disk = self._create_disk(
        ...
        self._anonymous_function_access = self._give_anonymous_function_access(self.function)
    ...
    def _get_image(self, image_family: str, image_project: str) -> AwaitableGetImageResult:
        return get_image(family=image_family, project=image_project)

    def _create_disk(self, disk_size: int, disk_type: str, image: AwaitableGetImageResult) -> Disk:
        return Disk(
    ...
    def _create_code_bucket(self) -> Bucket:
        return Bucket("pufferpanel-bucket", force_destroy=True, location="US")

    def _create_code_bucket_object(self, bucket: Bucket) -> BucketObject:
        return BucketObject(
    ...
    def _give_anonymous_function_access(self, function: Function) -> FunctionIamMember:
